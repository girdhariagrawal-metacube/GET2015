...
public class VehicleDBHelper {
	...
	protected void createVehicle(Connection connection, Vehicle vehicle)
		...
		try {
			...
			preparedStatement.setTimestamp(9, new Timestamp(vehicle
					.getCreatedTime().getTime()));

			preparedStatement.execute();
		} catch (SQLException e) {
			throw new MetaHRMSystemException("Could not create Vehicle, ["
					+ e.getMessage() + "]");
		} finally {
	...
	protected void updateVehicle(Connection connection, Vehicle vehicle)
			throws MetaHRMSystemException {
		PreparedStatement preparedStatement = null;
		try {
			preparedStatement = connection
			...
			throw new MetaHRMSystemException("Could not update Vehicle, ["
					+ e.getMessage() + "]");
		} finally {
			if (preparedStatement != null) {
				try {
